...
public class TopCode {
	...
	/** smarichal addon. Minimun distance to consider 2 markers with same id as the same marker.
	 * In a single frame this is not possible, but it is useful for tracking history and to add robustness to 
	 * detection algorithm for interactive systems. For example, a marker is no longer visible when is not detected 
	 * in N consecutives frames. To compute this algorithm we need to identify the marker as unique using the code and the
	 * position */
   private static final double MIN_MARKER_DISTANCE = 20;
   ...
   public void annotate(Graphics2D g, Scanner scanner) {
	      ...
	      for (int sector=SECTORS-1; sector>=0; sector--) {
	         ...
	         for (int i=3; i<WIDTH; i++) {
	            ...
	            g.setColor((sample == 0) ? java.awt.Color.BLACK : java.awt.Color.WHITE);
	 ...
	 */
	   public void drawDesktop(Graphics2D g) {
	      ...
	      for (int i=SECTORS-1; i>=0; i--) {
	         ...
	         g.setColor(((bits & 0x1) > 0)? java.awt.Color.white : java.awt.Color.black);
	         g.fill(arc);
	         bits >>= 1;
	      }
	   ...
	   public void drawDesktopCenter(Graphics2D g){
		   String center = "("+(int)x+","+(int)y+")";
		   g.setFont(new Font("TimesRoman", Font.PLAIN, 10)); 
		   g.drawString(center, x, y - WIDTH * 0.5f * unit);
	   }
	   
	   public void drawDesktopOrientationDegrees(Graphics2D g){
		   String orientationStr = String.valueOf(Math.round(Math.toDegrees(orientation))) + "º";
		   g.setFont(new Font("TimesRoman", Font.PLAIN, 10)); 
		   g.setColor(java.awt.Color.RED);
		   g.drawString(orientationStr, x, y + WIDTH * 0.5f * unit);
	   }
	   
	   public void drawDesktopOrientationRadians(Graphics2D g){
		   String orientationStr = String.valueOf(orientation);
		   g.setFont(new Font("TimesRoman", Font.PLAIN, 10)); 
		   g.setColor(java.awt.Color.RED);
		   g.drawString(orientationStr, x, y + WIDTH * 0.5f * unit);
	   }
